...
```
  <div align=center>
```{r,echo=FALSE}
qplot(long,lat,data = us.cities,color = I('blue')) + borders("state",size = 0.2)
fig <- plot_geo(df_500, locationmode = 'USA-states',colorscale = 'Portland')
fig <- fig %>% 
  layout(
    title = 'Sleeping less than 7 hours among adults'
  )

fig <- fig %>% 
  add_trace(
    type = 'choropleth',
    locations = df_500$CityName,
    z = ~df_500$CityName,
    text = ~CityName,
    colorscale = "Reds",
    colorbar(title = "Sleep deficiency Rate (%)")
)
```
  <div align=center>
```{r}
...
fig_plotly <- plot_ly(
    x = df_500$latitude,
    y = df_500$longitude,
    color = df_500$StateDesc,
    text = df_500$CityName,
    alpha = .2,
    mode = "markers"
  )
fig_plotly
