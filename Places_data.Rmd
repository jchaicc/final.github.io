---
title: "Correlation between prevalance of insufficient sleep and physiological disease"
author: "Cary Ni"
output: 
  html_document:
    code_folding: hide
...
data_df %>% 
  ...
  arrange(-population) %>% 
  filter(
    state_abbr %in% c('CA', 'TX', 'NY', 'FL', 'IL', 'PA' )
  ) %>% knitr::kable()
```

The dataset has `r nrow(data_df)` observations across the country, each represents a summarized estimate of a district. We select New York state as our sample data to build the model as it has third largest population with sufficient amount of sample. 
