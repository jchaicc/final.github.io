...
```{r}
xgb2 = xgboost(data = dtrain, max.depth = 5, nround = 300, objective = "binary:logistic")
print(xgb2)
```
...
#### testing
```{r}
xgpred = round(predict(xgb2, dtest))
...
```{r}
# xgb.dump(xgb2, with_stats = T)
```

```{r}
# xgpt = xgb.plot.tree(model = xgb2, trees=0:1, render=FALSE)
